...
class Config(object):
    ...
    def __init__(self):
        ...
        # 模型及路径配置
        self.initial_pretrain_model = 'bert-uncased'           # 加载的预训练分词器checkpoint，默认为英文。若要选择中文，替换成 bert-base-chinese
        self.initial_pretrain_tokenizer = 'bert-uncased'       # 加载的预训练模型checkpoint，默认为英文。若要选择中文，替换成 bert-base-chinese
        self.path_model_save = './checkpoint/5/'                      # 模型保存路径
        self.path_datasets = './datasets/dataset_1/'                          # 数据集
        self.path_log = './logs/'
