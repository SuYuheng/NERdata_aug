...
class DataManager(object):
    ...
    def __init__(self, config):
        ...
        self.ip_regex = r'\b\d{1,3}(\.\d{1,3}){3}\b'
        self.ip_regex_1 = r'\b\d{1,3}(?:\[.\]\d{1,3}){3}\b'
        self.domain_regex = r'\b[\w-]+(?:\[.\][\w-]+)+\b'
        self.url_regex = r'http[s]?://\S+'
        self.url_regex_1 = r'HXXP[S]?://\S+'
        self.hash_regex = r'\b([a-fA-F\d]{32}|[a-fA-F\d]{40}|[a-fA-F\d]{64})\b'
        # 定义正则表达式
    ...
    def get_dataset(self, mode='train', sampler=True):
        ...
        if mode == 'train':
            ...
            eval_dataloader = self.data_process('test.txt', tokenizer)
    ...
    def data_process(self, file_name, tokenizer, sampler=True):
        ...
        # tokenizer.
        tokenized_datasets = raw_datasets.map(lambda x: self.tokenize_function(x, tokenizer),
                                              batched=True)  # 对于样本中每条数据进行数据转换
        ...
        if sampler:
            ...
            sampler = None
        dataloader = DataLoader(tokenized_datasets, sampler=sampler,
                                batch_size=self.config.batch_size)  # , collate_fn=data_collator , num_workers=2, drop_last=True
        return dataloader

    def custom_tokenize(self, text):
        # print(text)
        # 替换IP地址
        text = re.sub(self.ip_regex, '<IP>', text)
        text = re.sub(self.ip_regex_1, '<IP>', text)
        # 替换URL
        text = re.sub(self.url_regex, '<URL>', text)
        text = re.sub(self.url_regex_1, '<URL>', text)
        # 替换域名
        text = re.sub(self.domain_regex, '<DOMAIN>', text)
        # 替换HASH值
        text = re.sub(self.hash_regex, '<HASH>', text)
        return text
    ...
    def tokenize_function(self, example, tokenizer):
        ...
        """
        # print(example["src"])
        for index, value in enumerate(example["src"]):
            example["src"][index] = self.custom_tokenize(value)
        # print(example["src"])
        # 分词
        token = tokenizer(example["src"], truncation=True, max_length=self.config.sen_max_length,
                          padding='max_length')  # config.padding)
